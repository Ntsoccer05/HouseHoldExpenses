...
**このスキルは、呼び出され際に以下の優先順で実行モードを自動判定します：**

```
1. tasklist.md が存在しない または 空である
   → モード1（ステアリングファイル作成）を実行
   
2. tasklist.md 内に未完了タスク `[ ]` が存在する
   → モード2（実装）を実行
   
3. tasklist.md の全タスクが完了 `[x]` かつ 振り返りセクションが空
   → モード3（振り返り）を実行
   
4. 上記のいずれでもない（例: 振り返りが既に記入されている）
   → ユーザーに対話形式でモード選択を提示
```

**ユーザーはモードを明示的に指定する必要はありません。スキル側で自動判定されます。**
