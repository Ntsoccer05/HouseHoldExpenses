...
このエージェントを起動する前に、以下のドキュメントが `docs/` に存在することを確認してください：
...
**未作成の場合の対処法**:
1. `/setup-project` コマンドを実行して、上記ドキュメントを作成してください
2. それでも存在しない場合は、関連する `.steering/[日付]-[機能名]/design.md` ファイルを代わりに参照してください
